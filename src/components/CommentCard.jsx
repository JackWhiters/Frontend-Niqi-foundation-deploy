...
const CommentCard = ({ index, leftVal, commentData }) => {
  ...
  let handleReplyClick = () => {
    if (!access_token) {
      return toast.error("Login dahulu sebelum membalas");
    }
        ...
        <div className="flex gap-5 items-center mt-5">
          {commentData.isReplyLoaded ? (
            <button
              ...
              onClick={hideReplies}
            >
              <i className="fi fi-rs-comment-dots"></i>Sembunyikan Balasan
            </button>
          ) : (
            <button
              className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2"
              onClick={loadReplies}
            >
              <i className="fi fi-rs-comment-dots"></i> {children.length} Balasan
            </button>
          )}
