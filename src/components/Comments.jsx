...
const CommentsContainer = () => {
  ...
  const loadMoreComments = async () => {
    ...
    console.log("New Comment Array: ", newCommentArr);
    console.log("Total Parent Comments Loaded: ", totalParentCommentsLoaded);
  ...
  return (
    <div
      className={
        ...
        " duration-700 mx-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"
      }
    >
      ...
      {commentsArr && commentsArr.length ? (
        ...
        <NoDataMessage message="Tidak Ada Komentar" />
      )}

      {total_parent_comments > totalParentCommentsLoaded ? (
        <button
          onClick={loadMoreComments}
          className="text-dark-grey p-2 px-3 hover:bg-grey30 rounded-md flex items-center gap-2"
        >
          Tampilkan Lebih
        </button>
      ) : (
